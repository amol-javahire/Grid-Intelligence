-- ============================================================
-- Populate ercot_hub_da_rt_hourly from the hourly ercot_nodal_da_rt_hourly seed.
--
-- ercot_hub_da_rt_hourly was empty on Azure, which broke:
--   • PyPSA Battery Revenue sim  ("No hourly data for HB_PAN 2025-06")
--   • ERCOT Historical → Hourly Shape tab
--   • (originally the capture MV, now repointed to ercot_nodal_da_rt_hourly)
--
...
--
-- Idempotent: DELETE + re-INSERT the 2025+ window that ercot_nodal_da_rt_hourly covers.
-- ============================================================
...
CREATE UNIQUE INDEX IF NOT EXISTS ercot_hub_da_rt_hourly_uq
  ON ercot_hub_da_rt_hourly (node, year, month, day, hour);
CREATE INDEX IF NOT EXISTS ercot_hub_da_rt_hourly_node_idx ON ercot_hub_da_rt_hourly (node);
CREATE INDEX IF NOT EXISTS ercot_hub_da_rt_hourly_time_idx ON ercot_hub_da_rt_hourly (year, month, day, hour);

DELETE FROM ercot_hub_da_rt_hourly WHERE year >= 2025;
